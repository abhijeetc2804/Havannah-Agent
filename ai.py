...
class Node4:
    ...
    def best_child(self, player, exploration_weight=1.414):
        choices_weights = [
    ...
    def expand(self, sim):
        ...
        for action1 in legal_actions:
            next_state = self.state.take_action(action1, self.player)
            child_node = Node4(next_state, 3 - self.player, self)
...
class MCTS4:
    def __init__(self, state, exploration_weight=0.7, rave_constant=500, ppr_threshold=0.4, ppr_probability=0.9, min_rave_visits=30):
        ...
        self.root = Node4(state,initial_player)

    def search(self, current_state, player, num_simulations=5000):
        print(starting_time)
        if starting_time>295:
            num_simulations=8000
        else:
            num_simulations=4000
        root = Node4(current_state, player)
        init_root = Node4(current_state, player)
        self.ppr_cache = {}
    ...
    def best_uct_child(self, node):
        ...
        for child in node.children:
            ...
            if combined_score > best_score:
                ...
                best_child = child
        #print("best_score: ",best_score-best_child_bonus/factor,best_child_bonus,best_child_bonus/75)
        return best_child
    ...
    def best_action(self, node):
        return max(node.children, key=lambda child: child.visits).state
...
class State4:
    ...
    def take_action(self, action, player):
        ...
        new_state = State4(new_grid,action)
        return new_state
...
class Node6:
    ...
    def expand(self, sim):
        ...
        for action1 in legal_actions:
            ...
            child_node = Node6(next_state, 3 - self.player, self)
            self.children.append(child_node)
...
class MCTS6:
    def __init__(self, state, exploration_weight=0.1, rave_constant=700, ppr_threshold=0.25, ppr_probability=0.95, min_rave_visits=15):
        ...
        self.root = None
        self.ppr_cache = {}  # New cache for PPR actions
        self.root = Node6(state,initial_player)
    ...
    def search(self, current_state, player, num_simulations=3000):
        ...
        # #print("init grid\n",init_bonus_grid)
        root = Node6(current_state, player)
        init_root = Node6(current_state, player)
        self.ppr_cache = {}
        ...
        # print("exp_time",exp_time, "move: ", moves_done)
        occupied_corners=init_occupied_corners.copy()
        occupied_edges=init_occupied_edges.copy()
...
class State6:
    ...
    def take_action(self, action, player):
        new_grid = self.grid.copy()
        new_grid[action] = player
        new_state = State6(new_grid, action)
...
class AIPlayer:
    ...
    def __init__(self, player_number: int, timer):
        ...
        starting_time= fetch_remaining_time(self.timer, self.player_number)
    ...
    def get_move(self, state: np.array) -> tuple:
        ...
        time_rem=fetch_remaining_time(self.timer, self.player_number)
    ...
    def get_move4(self, state: np.array) -> tuple:
        current_state = State4(state)
        count = 0
        for i in range(current_state.grid.shape[0]):
            for j in range(current_state.grid.shape[1]):
                if current_state.grid[i, j] == initial_player:
        ...
        legal_actions = current_state.get_legal_actions()
        for action in legal_actions:
            simulated_state = current_state.take_action(action, self.player_number)
            if simulated_state.is_terminal(action, self.player_number):
                print("immediate win ",action)
                return action  # Play the winning move immediately
        ...
        opponent = 3 - self.player_number
        for action in legal_actions:
            simulated_state = current_state.take_action(action, opponent)
            if simulated_state.is_terminal(action, opponent):
                print("forced block ",action)
                return action  # Block opponent's winning move
            
        # Step 3: Check for "Mate in 2" for AI
        mate_in_2_move = self.find_mate_in_2_size_4(self.player_number, current_state)
        if mate_in_2_move is not None:
            print("mate in 2 ",mate_in_2_move)
            return mate_in_2_move

        # Step 4: Prevent "Mate in 2" for the opponent
        prevent_mate_in_2_move = self.prevent_mate_in_2_size_4(current_state)
        if prevent_mate_in_2_move is not None:
            print("prevent mate in 2 ",prevent_mate_in_2_move)
            return prevent_mate_in_2_move

        mcts = MCTS4(current_state)
        best_state = mcts.search(current_state, self.player_number)
    ...
    def find_mate_in_2_size_4(self, player_to_move, current_state: State4) -> tuple:
        """ Find a move that guarantees a win in 2 moves (Mate in 2). """
        legal_actions = current_state.get_legal_actions()

        for action in legal_actions:
            # Simulate the AI's move
            new_state = current_state.take_action(action, player_to_move)

            if new_state.is_terminal(action, player_to_move):
            ...
            for opponent_action in opponent_legal_actions:
                ...
                opponent_state = new_state.take_action(opponent_action, opponent)
                ...
                for ai_action in ai_next_actions:
                    next_state = opponent_state.take_action(ai_action, player_to_move)
                    if next_state.is_terminal(ai_action, player_to_move):
                        ai_wins_next = True
                        break
    ...
    def prevent_mate_in_2_size_4(self, current_state: State4) -> tuple:
        ...
        for action in legal_actions:
            # Simulate AI's move
            new_state = current_state.take_action(action, self.player_number)
            if new_state.is_terminal(action,self.player_number):
                continue
            final_action = self.find_mate_in_2_size_4(opponent,new_state)
...
#### Correct vc
    ...
    def find_mate_in_2(self, player_to_move, current_state: State6) -> tuple:
        """ Find a move that guarantees a win in 2 moves (Mate in 2). """
        legal_actions = current_state.get_legal_actions()
        for action in legal_actions:
            # Simulate the AI's move
    ...
    def prevent_mate_in_2(self, current_state: State6) -> tuple:
        """ Check if opponent has a mate in 2 and block it if possible. """
        legal_actions = current_state.get_legal_actions()
